...
test.each(testCases)(
  ...
  (_, stringified, objectified) => {
    expect(qs.parse(stringified, sepratorCharacter, equalCharacter)).toStrictEqual(objectified);
  }
);
...
test.each(testCases)(
  ...
  (_, stringified, objectified) => {
    expect(qs.stringify(objectified, sepratorCharacter, equalCharacter)).toStrictEqual(stringified);
  }
);
